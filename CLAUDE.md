...
- `/af/:reqId/design` — run `af-design-boundaries` through the Stage Runner panel, then use the Graph IR review (currently 2-pane: module sidebar + a wide canvas — the right Inspector pane for contract editing / anchored-comment composing is parked via `INSPECTOR_ENABLED=false` in `DesignWorkbench` to give the graph full width; comments still persist under `collaboration/comments.json`). Toggles `boundaries_approved` when every module candidate is `status === "approved"` and Graph IR validation errors are zero.
