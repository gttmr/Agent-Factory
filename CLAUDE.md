...
- `/af/:reqId/design` — run `af-design-boundaries` through the Stage Runner panel, then use the Graph IR review. The review step is a top/bottom split (`af-design-split`): the **top** row is `[선택 노드/엣지 정보 패널 | wide canvas]` — the left panel shows only the selected node/edge detail via the reused `GraphInspector`; the **bottom** is a new full-width panel (`af-design-bottom`) holding the module/Graph IR/Runtime·A2A contract/경로/Comments tabs. The separate right Inspector pane for contract editing / anchored-comment composing stays parked via `INSPECTOR_ENABLED=false` in `DesignWorkbench`; comments still persist under `collaboration/comments.json`. Toggles `boundaries_approved` when every module candidate is `status === "approved"` and Graph IR validation errors are zero.
