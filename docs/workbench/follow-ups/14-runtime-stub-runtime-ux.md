...
상태: **부분 대체.** dependency 재설치 문제는 artifact-local venv를 폐기하고 공유 venv(`.agent-factory/runtime/.venv` / `AF_ADK_VENV_DIR`) + `requirements/adk-runtime.txt` 수동 준비 정책으로 대체했다. stale reload, adapter 없는 Mock Lab 패널 같은 UX 항목은 아직 별도 후속이다.

## 왜 필요한가 (관찰된 마찰)

1. **의존성 재설치 누락(대체됨)**: RunSandbox가 더 이상 `runtime-stub/.venv`를 만들지 않는다. 웹은 설치를 수행하지 않고, smoke/runnable/A2A/MCP를 포괄하는 공유 venv를 repo 기준 `requirements/adk-runtime.txt`로 수동 준비한다.
...
1. **deps 재설치 트리거(대체됨)**: artifact별 `requirements.txt`/`.venv` 재설치 추적 대신 공유 `requirements/adk-runtime.txt`와 수동 venv 준비 정책을 사용한다.
