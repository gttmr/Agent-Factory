...
- runnable lowering 지원 범위(ADK 2.x 그래프 `Workflow`): input/output, module-bound agent/adapter/workflow 노드, 병렬 fan-out + 명시/자동 `JoinNode` fan-in, 그리고 `human_input` 노드(`RequestInput`). `human_input`은 런타임에서 long-running `adk_request_input` 으로 pause 되고 동일 id의 `functionResponse`(`{response: ...}`)로 resume 된다. 엣지의 데이터 전달 방식 중 `session_state`/`temp_state`/`user_state`/`app_state` 채널은 `state_key`(스코프 prefix 자동)로 lower된다 — producer가 그 키에 기록(agent 단일 채널이면 `output_key`, function 노드는 `ctx.state[키]` 미러)하고 connected adapter consumer가 `_collect_tool_inputs`의 명명 채널에서 우선 읽으며, 채널 미지정 엣지는 기존 `{id}_output` 컨벤션으로 fallback해 동작이 불변이다. agent의 상이한 다중 out-state 키는 거부한다. `artifact` 채널도 lower된다 — function 노드가 payload를 JSON `types.Part`로 `save_artifact`하고 connected consumer가 `load_artifact`로 읽는다(import는 artifact 사용 시에만 추가). agent가 만든 artifact 출력은 거부한다. agent-consumer 명명 읽기는 후속이다. `loop_control`/`route`/`loop_back`/`loop_exit`/`conditional`(루프·라우터), `remote_a2a`, `dynamic_workflow`는 아직 lower 하지 않고 generator가 runnable에서 거부한다(smoke 사용). ADK 2.x 기준 반복 루프는 dynamic workflow 영역으로, 별도 후속 작업이다.
