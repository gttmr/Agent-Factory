...
- runnable lowering 지원 범위(ADK 2.x 그래프 `Workflow`): input/output, module-bound agent/adapter/workflow 노드, 병렬 fan-out + 명시/자동 `JoinNode` fan-in, 그리고 `human_input` 노드(`RequestInput`). `human_input`은 런타임에서 long-running `adk_request_input` 으로 pause 되고 동일 id의 `functionResponse`(`{response: ...}`)로 resume 된다. `loop_control`/`route`/`loop_back`/`loop_exit`/`conditional`(루프·라우터), `remote_a2a`, `dynamic_workflow`는 아직 lower 하지 않고 generator가 runnable에서 거부한다(smoke 사용). ADK 2.x 기준 반복 루프는 dynamic workflow 영역으로, 별도 후속 작업이다.
