...
- 기존 manual Build/Verify controls는 별도/advanced 경로로 유지된다. `POST /api/af/:id/runtime-stub/build`는 저장된 `scaffold-plan.json` 기준으로 generator만 실행하고, `/af/:reqId/verify`의 `POST /api/af/:id/verify/run`은 고정 allow-list command를 독립 실행한다. 새 기능을 추가할 때 이 수동 경로가 compound action의 server contract와 다른 순서를 만들면 안 된다.
