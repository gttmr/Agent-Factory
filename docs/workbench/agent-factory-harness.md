...
- `/af/:reqId/design` runs `af-design-boundaries` only when `analysis_reviewed === true`, then mounts a 3-pane Design workbench (left tabs `모듈 / Graph IR / Comments`, center `GraphCanvas`, right inspector with node/edge-anchored comment thread). The `boundaries_approved` gate enables only when every module candidate is `status === "approved"` and Graph IR validation errors are zero. `runtime_contracts_approved` is currently flipped via direct manifest PATCH until a dedicated contract review UI ships.
