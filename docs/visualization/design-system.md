...
- `카탈로그 워크플로우 삽입` 버튼은 편집 모드 draft와 별개다. picker modal은 `/api/catalog`의 workflow 항목을 이름, owner domain, version/status, responsibility로 보여주고, 선택 시 단일 `workflow` 노드와 matching `ModuleCandidate`를 `analysis-result.json`에 즉시 저장한다. `runtime_binding: remote_a2a` 또는 `component_source: remote_a2a` workflow 항목은 예외적으로 `a2a_provider_req_id`로 local provider Agent Card를 불러와 기존 local A2A provider import 경로를 통해 `remote_a2a` facade 후보, A2A 계약, remote graph node/edge를 추가한다. provider id가 없거나 Agent Card fetch가 실패하면 visible notice만 표시하고 analysis 저장은 하지 않는다. 이 기능은 catalog workflow fragment를 확장하지 않고 재사용 workflow를 하나의 Graph IR node 또는 Remote A2A facade node로 추가한다.
...
- `human_input` 노드의 `human_input_contract`는 편집 패널 계약 탭에서 노출한다. Reviewer는 `RequestInput message`, payload schema ref, response schema ref, response mapping을 입력하며, 현재 runnable skeleton이 자동 lower하는 response schema는 `str` 또는 빈 값이다.
- `route` 엣지는 편집 패널 계약 탭에서 `route_condition`, `route_aliases`, `router fallback route` 체크박스를 노출한다. Alias는 줄 단위로 입력하고, 같은 router에서 fallback은 하나만 선택해야 한다.
...
- 엣지 선택 시 `edge_kind`, `execution_semantics`, `data_label`, `schema_ref`, `route_condition`, `route_aliases`, `is_default_route`, `state_key`, `artifact_key`, `a2a_contract_id`, boundary crossing을 표시한다.
